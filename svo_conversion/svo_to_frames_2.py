...
def main():
    ...
    while True:
        #err = cam.grab(runtime)
        if cam.grab() == sl.ERROR_CODE.END_OF_SVOFILE_REACHED:
            print('done reading file')
            cam.close()
            return
        if cam.grab() != sl.ERROR_CODE.SUCCESS:
            print('unkown error with cam.grab()')
            cam.close()
            return
        #outfile = f"{outfilepath}/{filename}_{i}.{filetype}"
        #i += 1
        ...
        #    cam.close()
        #    return
        for view in [sl.VIEW.LEFT, sl.VIEW.RIGHT]:
            outfile = f"{outfilepath}/{filename}_{i}.{filetype}"
            i += 1
